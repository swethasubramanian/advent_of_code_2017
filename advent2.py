...
inp_final = """1236	741	557	1029	144	101	1968	2159	1399	80	1139	1167	1695	82	90	2236
...
def parse_data(inp):
	x  = inp.split("\n")
	x_mat = [el.split("\t") for el in x]
	x_array = np.array(x_mat).astype(int)
	return x_array

def div_is_whole(slice):
	for el in slice:
		num = slice[np.where(slice%el == 0)]
		if len(num) > 1:
			return max(num)/min(num)


def checksum1(x_array):
	return (np.max(x_array, axis = 1) - np.min(x_array, axis = 1)).sum()

def checksum2(x_array):
	x_array_sorted = np.sort(x_array, axis = 1)
	x = np.apply_along_axis(div_is_whole, 1, x_array_sorted)
	return np.sum(x)

def solve(inp):
	x_array = parse_data(inp)
	res_1 = checksum1(x_array)
	res_2 = checksum2(x_array)
	print res_1, res_2
	return res_1, res_2
